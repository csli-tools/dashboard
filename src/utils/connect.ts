...
const helpFileTicketPlz = (u: any) => {
  ...
  console.error('https://github.com/csli-tools/dashboard/issues/new')
}
...
export const attemptConnect = async () => {
  ...
  try {
    ...
    wss.on('connection', function connection(ws) {
      ws.on('message', function message(data) {
        // Message received from breakout pane
      });
    });
  } catch (e) {
  ...
  try {
    ...
    if (errorMessage.includes('ECONNREFUSED')) {
      console.error(chalk.red(`Couldn't connect to NEAR RPC. Is the network accessible?\nWe're trying to connect to ${chalk.yellow(getNEARConfig().nodeUrl)}\nPlease update the ${chalk.yellow('NEAR_NETWORK')} environment variable in the ${chalk.yellow('.env')} file to 'mainnet', 'testnet', or 'localnet'`))
    } else if (errorMessage.includes('ENOTFOUND')) {
      console.error(chalk.red(`Couldn't resolve NEAR RPC host. Please check your internet connection.\nWe're trying to connect to ${chalk.yellow(getNEARConfig().nodeUrl)}`))
    } else if (errorMessage.includes('ETIMEDOUT')) {
      console.error(chalk.red(`Connection to NEAR RPC timed out. The network may be slow or unavailable.\nWe're trying to connect to ${chalk.yellow(getNEARConfig().nodeUrl)}`))
    } else {
